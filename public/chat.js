...
const CHAT_STORAGE_KEY="fin_chat_history_v5";
const CONVERSATION_ID_KEY="fin_conversation_id_v1";
const MAX_STORED_MESSAGES=30;
const WELCOME="Hi! I’m Fin. Tell me what changed, or ask me to pull something up from your planner. ✦";
...
function replayClass(el,name,duration){if(!el)return;el.classList.remove(name);void el.offsetWidth;el.classList.add(name);window.setTimeout(()=>el.classList.remove(name),duration)}
function runFinAnimation(name){
  if(name==="income-success"){
    replayClass(fin,"income-celebrate",1150);
    replayClass(magicBook,"flip",1450);
  }else if(name==="notion-success"){
    replayClass(fin,"success-pop",850);
  }
}

function openVisualization(v){
  if(!v||!window.FinVisualizations)return;
  insightTitle.textContent=v.title||"Your breakdown";
  insightSubtitle.textContent=v.subtitle||"";
  insightContent.innerHTML=window.FinVisualizations.render(v);
  app.classList.add("insight-open");
  insightPanel.setAttribute("aria-hidden","false");
  setFinState("visualizing");
}
function closeVisualization(){
  app.classList.remove("insight-open");
  insightPanel.setAttribute("aria-hidden","true");
  if(!isProcessing)setFinState("idle");
}
closeInsight?.addEventListener("click",closeVisualization);

function renderConversation(){
  chatMessages.innerHTML="";
  if(!chatHistory.length){addMessage("assistant",WELCOME);chips.hidden=false;return}
  chatHistory.forEach(m=>addMessage(m.role,m.content));
  chips.hidden=true;
}
renderConversation();
window.addEventListener("load",()=>{if(!window.matchMedia("(prefers-reduced-motion: reduce)").matches)replayClass(fin,"enter",1100)});
...
async function sendMessage(){
  ...
  try{
    ...
    const append=content=>{if(!content)return;responseText+=content;renderRichAssistant(el,responseText);typingIndicator?.classList.remove("visible");if(!app.classList.contains("insight-open"))setFinState("talking");scrollBottom()};
  ...
  finally{typingIndicator?.classList.remove("visible");if(!app.classList.contains("insight-open"))setFinState("idle");isProcessing=false;userInput.disabled=false;sendButton.disabled=false;userInput.focus()}
}
...
function handleEvents(events,appendText){
  for(const data of events){
    ...
    try{
      ...
      if(json.type==="visualization"&&json.visualization){openVisualization(json.visualization);continue}
      if(json.type==="animation"&&json.name){runFinAnimation(json.name);continue}
      const content=typeof json.response==="string"?json.response:json.choices?.[0]?.delta?.content||json.content||"";
      if(content)appendText(content);
    }catch(error){console.warn("Could not parse Fin stream:",data,error)}
  }
  return false;
}
